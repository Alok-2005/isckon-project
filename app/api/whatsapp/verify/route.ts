...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!from || !message) {
      ...
      await sendWhatsAppMessage(from || "whatsapp:+1234567890", "Invalid request. Please provide a valid message.");
    ...
    if (!transactionIdMatch) {
      ...
      await sendWhatsAppMessage(from, "Invalid message format. Please include the Transaction ID.");
      clearTimeout(timeoutId);
    ...
    if (!payment) {
      ...
      await sendWhatsAppMessage(from, "Payment not found or not completed. Please check your Transaction ID.");
      clearTimeout(timeoutId);
    ...
    const { fileName, pdfUrl } = await generateReceiptPDF(payment, transactionId);
